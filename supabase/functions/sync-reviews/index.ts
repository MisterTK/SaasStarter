  ...
  try {
    ...
    for (const org of organizations || []) {
      try {
        ...
        const locations = await gmb.getAllAccessibleLocations(org.organization_id)
        ...
        for (const location of locations) {
          ...
          const reviews = await gmb.getReviews(org.organization_id, accountId, locationId)
          
          // Filter for unanswered reviews
          const unansweredReviews = reviews.filter(review => !review.reviewReply)
