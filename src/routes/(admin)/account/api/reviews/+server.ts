...
export const GET: RequestHandler = async ({ locals: { safeGetSession, supabaseServiceRole }, url, cookies }) => {
  ...
  if (!accountId || !locationId) {
    return json({ error: 'Missing accountId or locationId parameter' }, { status: 400 })
  }
  ...
  try {
    ...
    return json({ error: error instanceof Error ? error.message : 'Failed to fetch reviews' }, { status: 500 })
  }
}
...
export const POST: RequestHandler = async ({ locals: { safeGetSession, supabaseServiceRole }, request, cookies }) => {
  ...
  const { accountId, locationId, reviewId, action, replyText } = await request.json()
  ...
  try {
    ...
    switch (action) {
      case 'reply':
        ...
        success = await service.replyToReview(accountId, locationId, reviewId, replyText)
        break
      case 'delete_reply':
        success = await service.deleteReviewReply(accountId, locationId, reviewId)
        break
      default:
    ...
    return json({ error: error instanceof Error ? error.message : 'Failed to manage review' }, { status: 500 })
  }
}