  ...
  try {
    ...
    for (const location of locations) {
      try {
        ...
        if (!locationId) {
          results.push({ locationId: location.locationId || 'unknown', error: "Missing locationId" })
          continue
        }

        // Use the improved getReviews method which handles wildcard fallback
        // If accountId is missing or same as locationId, it will use wildcard
        ...
        for (const review of reviews) {
          ...
          const { error } = await supabaseServiceRole.from("reviews").upsert({
            ...
            location_name:
              location.locationName || review.locationName || locationId,
