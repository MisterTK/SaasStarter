...
const GOOGLE_MY_BUSINESS_REVIEWS_API =
...
export class GoogleMyBusinessService {
  ...
  private parseLocationName(locationName: string): LocationIdentifiers {
    ...
    // Handle just the ID
    return {
      locationId: locationName.replace('locations/', '').replace('accounts/', ''),
      fullName: undefined
    }
  }
  ...
  async getReviewsWithWildcard(locationId: string): Promise<Review[]> {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error("Failed to fetch reviews using wildcard approach:", errorText)
        ...
        if (response.status === 404 || errorText.includes('NOT_FOUND')) {
          console.warn(`Location ${locationId} might not be accessible with current credentials`)
        }
        return []
      }
  ...
  async getReviews(accountId: string, locationId: string): Promise<Review[]> {
    ...
    if (cleanAccountId === cleanLocationId) {
      console.warn(`Account ID and Location ID are the same (${cleanAccountId}), this is likely an error. Using wildcard approach.`)
      return this.getReviewsWithWildcard(cleanLocationId)
    }
    ...
    const errorText = await response.text()
    console.warn(`Direct reviews fetch failed for account ${accountId}, location ${locationId}: ${errorText}`)
  ...
  async getReviewsByLocationName(locationName: string): Promise<Review[]> {
    ...
    if (identifiers.fullName && identifiers.accountId) {
      ...
      const errorText = await response.text()
      console.warn(`Direct reviews fetch failed for ${locationName}: ${errorText}`)
      
      // Check if it's an access issue vs other errors
      if (response.status === 403 || errorText.includes('PERMISSION_DENIED')) {
        console.log("Permission denied with direct access, trying wildcard approach...")
      }
    }
  ...
  async replyToReviewByName(
    ...
    if (response.status === 403 || errorText.includes('PERMISSION_DENIED')) {
      ...
      if (parts.length >= 6 && parts[0] === 'accounts' && parts[2] === 'locations' && parts[4] === 'reviews') {
  ...
  async getReviewsOptimal(locationInfo: string | { accountId?: string; locationId: string }): Promise<Review[]> {
  ...
  async checkLocationAccess(locationId: string): Promise<{ hasAccess: boolean; accessType?: 'direct' | 'wildcard' }> {
    const cleanLocationId = locationId.replace("locations/", "")
